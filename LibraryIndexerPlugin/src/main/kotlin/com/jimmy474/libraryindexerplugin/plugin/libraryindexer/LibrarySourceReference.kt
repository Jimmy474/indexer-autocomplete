...
class LibraryReferenceProvider: PsiReferenceProvider(){
    override fun getReferencesByElement(element: PsiElement, context: ProcessingContext): Array<PsiReferenceBase<PsiElement>> {
        ...
        val refs: MutableList<PsiReferenceBase<PsiElement>> = mutableListOf()
...
class LibrarySourceReference(
    ...
    override fun isReferenceTo(element: PsiElement): Boolean {
        if (element is com.intellij.psi.PsiClass) {
    ...
    private fun parametersMatch(method: PsiMethod, expectedTypes: List<String>?): Boolean {
        ...
        return actualTypes.zip(expectedTypes).all { (actual, expected) ->
            actual.canonicalText == expected ||
                    ...
                    actual.canonicalText.endsWith(expected) ||
                    expected.endsWith(actual.canonicalText) ||
                    expected.endsWith(actual.presentableText)
        }
    }
}
